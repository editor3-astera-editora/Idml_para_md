...
Entrypoint exposto pelo `pyproject.toml`:

- `idml2md` → `idml_to_md.cli:app` — conversão IDML → Markdown.

Usa [Typer](https://typer.tiangolo.com/); use `--help` em qualquer subcomando para ver a ajuda contextual.
...
O pipeline registra avisos via `loguru` (warnings) sem alterar o código de saída — falhas de assets, equações que caíram no fallback e estilos não mapeados aparecem em `_report.json`.

## Próximo

[configuration.md](configuration.md) — como customizar mapeamentos de estilos.
