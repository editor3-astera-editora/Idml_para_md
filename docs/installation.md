...
A flag `[dev]` traz `pytest`, `pytest-cov`, `pytest-xdist`, `pytest-mock`, `pytest-snapshot`, `ruff` e `mypy`. Sem ela, apenas as dependências de runtime (`simpleidml`, `lxml`, `typer`, `pydantic`, `pyyaml`, `loguru`, `rich`, `pillow`) são instaladas.

Após a instalação, um entrypoint fica disponível no PATH:
